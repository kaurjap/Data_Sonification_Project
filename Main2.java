...
public class Main2 implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        String timeSelected;
        timeSelected = (String) bottomPanel.getTime().getSelectedItem();
        // radio buttons with stock indexes
        if (bottomPanel.getNYSE().isSelected()) {
            reader.setTime(timeSelected);
            reader.setFilename("src/NYSE.csv");
            reader.readDataTo(stockprices);
        } else if (bottomPanel.getNASDAQ().isSelected()) {
            reader.setTime(timeSelected);
            reader.setFilename("src/NASDAQ.csv");
            reader.readDataTo(stockprices);
        } else if (bottomPanel.getDJIA().isSelected()) {
            reader.setTime(timeSelected);
            reader.setFilename("src/DJIA.csv");
            reader.readDataTo(stockprices);
        } else {
        ...
        String selected;
        selected = (String) bottomPanel.getInstruments().getSelectedItem();
        ...
        String priceType;
        priceType = (String) bottomPanel.getPriceToWatch().getSelectedItem();
        ...
        //myGraph.start(minNum, maxNum, rangeFrom, stockprices);
